"""
WordPress REST API page updater for krguide.com.
Updates: Home, Learn Korean, Contact + creates About
All wrapped in wp:html Gutenberg block to prevent editor mangling.
"""
import requests
import base64
import json
...
def wp_html_block(inner_html: str) -> str:
    """Wrap HTML in Gutenberg custom HTML block."""
    return f"<!-- wp:html -->\n{inner_html}\n<!-- /wp:html -->"
...
<p style="color:#666;margin:0 0 12px;">For questions about learning materials, payment, or license keys, please email us.</p>
...
<p style="color:#666;margin:0;">Payments are processed by Gumroad. Refunds can be requested directly from Gumroad within 7 days of purchase.</p>
</div>
...
<p style="color:#666;margin:0;">If you lost your license key, email hello@krguide.com with the address you used to purchase. We'll resend it.</p>
</div>
...
<h2 style="border-left:4px solid #C0392B;padding-left:14px;margin:32px 0 14px;">🇰🇷 Who We Are</h2>
<p>KR Guide is a complete Korean learning resource for foreigners. Beyond simple word lists, we provide <strong>example sentences, scenario dialogues, comparison boxes, mini quizzes, and pronunciation QR codes</strong> — a living, breathing Korean learning experience.</p>

<h2 style="border-left:4px solid #1A4A8A;padding-left:14px;margin:32px 0 14px;">📚 What We Offer</h2>
<ul style="line-height:2;">
<li><strong>41-chapter Main Guide</strong> — Hangul · pronunciation · grammar · conversations · culture</li>
<li><strong>14 categories · 6,546 words & expressions</strong> — verbs · nouns · honorifics · idioms · proverbs · and more</li>
<li><strong>Google Translate (13 languages)</strong> — English · Japanese · Chinese · Spanish · French and more</li>
<li><strong>Pronunciation QR codes</strong> — instantly hear any word</li>
</ul>

<h2 style="border-left:4px solid #059669;padding-left:14px;margin:32px 0 14px;">✨ Our Promise</h2>
<p><strong>One purchase · lifetime access</strong> — no subscription, no recurring fees. All future updates are included with your original license key.</p>
...
<h3 style="color:#fff;margin:0 0 12px;">Start Today</h3>
...
<a href="https://jssmn21.gumroad.com/l/gnefla" target="_blank" rel="noopener" style="display:inline-block;background:transparent;color:#fff;border:2px solid #fff;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:800;margin:4px;">💳 Buy</a>
</div>

</div>"""

def update_page(page_id, content, title=None):
    ...
    if r.ok:
        d = r.json()
        print(f"[OK] Updated page {page_id} ('{d['title']['rendered']}') → {d['link']}")
    else:
        print(f"[FAIL] Page {page_id} failed: {r.status_code} {r.text[:200]}")
    return r.ok

def create_page(slug, title, content, status="publish"):
    r = requests.post(
    ...
    if r.ok:
        d = r.json()
        print(f"[OK] Created page {d['id']} ('{d['title']['rendered']}') → {d['link']}")
        return d["id"]
    else:
        print(f"[FAIL] Create '{slug}' failed: {r.status_code} {r.text[:200]}")
        return None

# Run updates
print("=== Updating pages ===")
update_page(21, HOME_HTML, title="KR Guide — Learn Korean")
update_page(1147, LEARN_KOREAN_HTML, title="Learn Korean")
update_page(46, CONTACT_HTML, title="Contact")

print("\n=== Creating About page ===")
create_page("about", "About", ABOUT_HTML)
